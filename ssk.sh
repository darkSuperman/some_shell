...
build_client_options_string() {
    ...
    local r_mtu=$(json_value mtu 1350)
    local r_sndwnd=$(json_value sndwnd 1024)
    local r_rcvwnd=$(json_value rcvwnd 1024)
    ...
    local r_conn=$(client_only_value conn 2)
...
install_kcptun() {
    ...
    local r_sndwnd=$(prompt_uint "请输入发送窗口 sndwnd" 1024 1 65535)
    local r_rcvwnd=$(prompt_uint "请输入接收窗口 rcvwnd" 1024 1 65535)
    ...
    local r_conn=$(prompt_uint "请输入客户端 KCP 连接数 conn（仅写入 Android 插件参数）" 2 1 16)
