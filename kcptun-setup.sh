...
EOF
    ...
    echo -e " ${BLUE}加速响应模式 (Mode):${PLAIN} ${YELLOW}${r_mode}${PLAIN}"
    ...
    echo -e " KeepAlive:         ${r_keepalive}"
    echo -e " 总带宽 Mbps:       ${r_bandwidth_mbps}"
    echo -e " 出口网卡:          ${r_bandwidth_iface:-自动检测}"
    echo -e " 单连接限速 Mbps:   ${r_per_conn_limit_mbps}"
    echo -e " 客户端连接数 conn: ${r_conn}"
    echo -e " 自动重建 autoexpire: ${r_autoexpire}"
